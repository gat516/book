...
use crate::pb::{
    text_proc_server::TextProc, Alias, HashRequest, HashResponse, MentionScanRequest,
    MentionScanResponse, RetroRequest, RetroResponse, Span,
};
...
impl TextProcService {
    pub fn new(capacity: usize) -> Self {
        ...
        Self { matchers: Arc::new(Mutex::new(LruCache::new(capacity))) }
    }
    ...
    async fn matcher(&self, language: &str, aliases: &[Alias]) -> Result<Arc<Matcher>, Status> {
        ...
        for alias in aliases {
            if !alias.surface.is_empty() {
                surfaces.entry(alias.surface.clone()).or_default().push(alias.alias_id.clone());
            }
        }
        ...
        let matcher = Arc::new(Matcher {
            automaton,
            alias_ids: surfaces.into_values().collect(),
        });
        self.matchers.lock().await.put(key, matcher.clone());
        Ok(matcher)
    }
}
...
fn simhash64(text: &str) -> String {
    ...
    let shingles: Vec<String> = if chars.len() < 5 {
        ...
        chars.windows(5).map(|window| window.iter().collect()).collect()
    };
    ...
    let value = weights.iter().enumerate().fold(0u64, |result, (bit, weight)| {
        if *weight > 0 { result | (1 << bit) } else { result }
    });
...
impl TextProc for TextProcService {
    async fn scan_mentions(
        ...
        if request.text.len() > MAX_TEXT_BYTES || request.aliases.len() > MAX_ALIASES {
            return Err(Status::resource_exhausted("text or alias request exceeds service limit"));
        }
    ...
    async fn hash_content(&self, request: Request<HashRequest>) -> Result<Response<HashResponse>, Status> {
        let text = request.into_inner().text;
        if text.len() > MAX_TEXT_BYTES {
            return Err(Status::resource_exhausted("text request exceeds service limit"));
        }
    ...
    async fn apply_retro_update(&self, _: Request<RetroRequest>) -> Result<Response<RetroResponse>, Status> {
        Err(Status::unimplemented("ApplyRetroUpdate is deferred to Milestone 3"))
    }
}
...
mod tests {
    ...
    async fn scan_preserves_shared_aliases_and_leftmost_longest() {
        ...
        let response = service
            .scan_mentions(Request::new(MentionScanRequest {
                ...
                aliases: vec![
                    Alias { alias_id: "short".to_owned(), surface: "王".to_owned() },
                    Alias { alias_id: "long-a".to_owned(), surface: "王国".to_owned() },
                    Alias { alias_id: "long-b".to_owned(), surface: "王国".to_owned() },
                ],
                lang: "zh".to_owned(),
            }))
            .await
        ...
        assert_eq!((response.spans[0].byte_start, response.spans[0].byte_end), (12, 18));
        assert_eq!((response.spans[0].char_start, response.spans[0].char_end), (4, 6));
    }
    ...
    async fn hash_is_exact_and_retro_is_deferred() {
        ...
        let response = service
            .hash_content(Request::new(HashRequest { text: "abc".to_owned() }))
            .await
            .expect("hash")
            .into_inner();
        assert_eq!(response.sha256, "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad");
        assert!(response.near_dup_sig.starts_with("simhash64-v1:"));
        assert_eq!(
            service.apply_retro_update(Request::new(RetroRequest::default())).await.unwrap_err().code(),
