...
export function RecordList({ rows, status, onEntity, showChapter = false }: { rows: RecordView[]; status?: RecordsStatus; onEntity?: (id: string, surface: string) => void; showChapter?: boolean }) {
  if (!rows.length) return <p className="records-empty">{status?.extraction_status === "ready" ? "No records in this chapter." : status?.extraction_status === "failed" ? "Record extraction failed; retry is available in Book settings." : "Knowledge is still being extracted…"}</p>;
  return <section className="record-list" aria-label="Chapter records">
    {rows.map(row => <article className="record-card" key={row.id}>
      ...
      {!!row.participants.length && <p className="record-participants"><b>Participants:</b> {row.participants.map((person, i) => <span key={`${person.ordinal ?? i}-${person.surface}`}>{i > 0 && ", "}{person.entity_id ? <button type="button" onClick={() => onEntity?.(person.entity_id!, person.surface)}>{person.surface}</button> : <span className="record-unresolved" title={person.unresolved_reason ?? "Identity unresolved"}>{person.surface} <small>(unresolved)</small></span>}</span>)}</p>}
