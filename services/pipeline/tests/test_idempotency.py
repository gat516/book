...
under a still-valid key. 1.5 adds the second half of that argument — the *state* stage's
...
def _key(stage: str, cfg: Config, *, ontology: dict | None = ONTOLOGY) -> str:
    """The state stage requires an ontology; the others ignore one. Passing it
...
def test_provider_switch_changes_key():
    ollama = _key("state", _cfg(llm_provider="ollama"))
    anthropic = _key("state", _cfg(llm_provider="anthropic"))
    assert ollama != anthropic


def test_model_switch_changes_key():
    a = _key("state", _cfg(llm_model_extract="qwen2.5:14b"))
    b = _key("state", _cfg(llm_model_extract="claude-haiku-4-5"))
    assert a != b
...
def test_translate_key_differs_from_extract_key():
    ...
    assert _key("translate", cfg) != _key("state", cfg)
...
def test_ontology_edit_changes_state_key():
    ...
    assert _key("state", cfg) != _key("state", cfg, ontology=richer)
...
def test_ontology_omitted_for_state_raises():
    ...
    with pytest.raises(ValueError, match="ontology"):
        idempotency_key("state", RAW_HASH, _cfg())


def test_ontology_ignored_by_other_stages():
    """Only state's config version is the ontology. Translate's is the glossary (1.7);
...
def test_key_uses_unit_separator_not_pipe():
    ...
    assert _key("state", cfg) != pipe_digest
