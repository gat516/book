...
A record row whose story-time (valid_from_chapter=10) is far earlier than its
knowledge-time (source_chapter=500) — the reader learns of it at ch.500 even though it "happened" at
...
async def _visible_at(conn, novel_id: str, reader_chapter: int) -> list:
    rows = await (
        await conn.execute(
            "SELECT id::text FROM record_row WHERE novel_id = %s AND source_chapter <= %s",
            (novel_id, reader_chapter),
        )
    ).fetchall()
    return rows
...
async def test_flashback_hidden_before_knowledge_chapter(db_conn):
    ...
    try:
        ...
        rows = await _visible_at(db_conn, novel_id, 220)
        assert rows == []
    finally:
...
async def test_flashback_visible_after_knowledge_chapter(db_conn):
    ...
    try:
        fixture = await seed_flashback(db_conn, novel_id)
        rows = await _visible_at(db_conn, novel_id, 500)
        assert [r[0] for r in rows] == [fixture["row_id"]]
    finally:
        await delete_novel(db_conn, novel_id)
